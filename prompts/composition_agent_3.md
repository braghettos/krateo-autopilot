...
> `portal-composition-page-generic` is an helm chart for a generic portal section. The portal section refers to the frontend in Krateo. The Blueprint `portal-composition-page-generic` adds a Composition page to the Krateo Composable Portal, showcasing all informations that are auto-consistent and do not depend on the Composition itself. In other words, `portal-composition-page-generic` creates a section in the frontend of Krateo that shows the state of the composition.

> ALWAYS include the `portal-composition-page-generic` section to the `Chart.yaml` file. 
