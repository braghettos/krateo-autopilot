...
6. If any generated file is invalid, understand the issue reading the error message and generate the file again until all files are correct. 
...
Every widget has a `widgetData` property that contains data used to control how the widget looks like or behaves in the Frontend Composable Portal, in this example we are defining a `label`, an `icon` (using fontawesome) naming convention) and a `type` that control the the visual style of the button.
...
```
IMPORTANT:
In the example above, we declared a table with a single column name to display all namespaces in the cluster. The data is loaded directly from the k8s api server

The `.namespaces` variable contains the output of a call to the endpoint `/api/v1/namespaces`.
We then use `.namespaces` in the filter to extract only the content we need from the output of the api.

At the end of the filtering, the restaction will return a data structure that looks like this: 
...
```
In the Table above, we can apply a second jq filter in the expression, exctracting only the namespaces:
