...
Below is explained what a composition is and how it is structured.
...
> NOTE: for some compositions, the portal section may not exists on its own as in the example above, but could be put into the chart templates so that its values can be parametrized. This can be the case when the user asks for a portal section with a dynamic number of panels, tables, etc... In the latter case the structure becomes as follows:
...
> Never cahnge the passwordRef, it is Always the same.
...
> Make sure that all image references are lowercase! If the username is EmondDantes it should become edomnddantes!
...
4. Apply the frontend components 
...
2. Identify the requirements for the user-facing portal. What information should be displayed?
...
1. Determine Portal Location: Based on the user's request, decide where to place the portal files.
...
### Step 3: Generate the Portal Files 

Based on the knowledge provided below on the portal in Krateo, create all the necessary files for the portal section.
...
2. DELIVER A COMPLETE PACKAGE. The user should receive all the files needed to deploy the composition, from the compositiondefinition.yaml to the chart files to the portal files.
...
- `apply_composition_definition`: Applies a Composition Defintiion to the Kubernetes cluster.
...
As previously stated, use this function if and only if the user confirms that they want to apply the generated composition defintion.
---
